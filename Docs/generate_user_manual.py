...
def generate_manual():
    ...
    doc.add_paragraph()
    for label, value in [
        ("文件版本", f"{VERSION_TAG}（含出入料管理 / 硬體設定 / ABB機械手 / Modbus TCP 設備支援）"),
    ...
    simple_table(
        ...
        [
            ...
            [VERSION_TAG, REVIEW_DATE, "AI-assisted",    "新增第 6 章出入料管理（PR#73-74）；補日期篩選與效能強化說明（PR#75）；章節重新編號"],
        ],
        [2.0, 2.5, 3.5, 8.0]
    )
    doc.add_page_break()
    ...
    toc_items = [
        ...
        ("  7.5", "巡檢時段設定", ""),
        ("  7.6", "角色與權限管理", ""),
        ("  7.7", "系統參數設定（業務時間 / 排程 / 閒置登出）", ""),
        ("  7.8", "硬體設定（讀卡機 / ABB 機械手 RAPID 位址）", ""),
        ("  7.9", "日誌管理與上傳", ""),
        ("8.",     "硬體設定（讀卡機 / Modbus TCP 設備）", ""),
        ("  8.1",  "新增讀卡機", ""),
        ("  8.2",  "移除讀卡機", ""),
        ("  8.3",  "Modbus TCP 設備", ""),
        ("  8.3.1","新增 Modbus TCP 連線", ""),
        ("  8.3.2","讀寫測試平台", ""),
        ("  8.3.3","移除 Modbus TCP 設備", ""),
        ("9.", "離線模式說明", ""),
        ("10.", "登出與閒置保護", ""),
        ("11.", "常見問題（FAQ）", ""),
    ]
    ...
    # ══════════════════════════════════════
    heading1("8. 硬體設定（讀卡機 / Modbus TCP 設備）")
    ...
    heading3("8.3.3 移除 Modbus TCP 設備")
