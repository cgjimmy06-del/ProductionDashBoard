...
ALL_FIGURES = [
    ...
    ("F5-9",  "強制安排開關與品項篩選（狀態下拉 + 關鍵字搜尋）"),
    ("F6-1",  "圖表設計頁面總覽（頁籤 / 統計列 / 篩選列 / 卡片牆或表格容器）"),
    ("F6-2",  "圖表設計器（左側預覽 / 右側基本、統計列、篩選列、容器設定）"),
    ("F6-3",  "圖表操作列（複製 / 刪除 / 匯出 / 匯入按鈕群）"),
    ("F7-1",  "出入料管理主畫面（統計列 / 篩選列 / 排單清單 / 右側詳細資訊面板）"),
    ("F7-2",  "建立排單對話框（件號 / 型號 / 工序 / 數量 / 備註欄位）"),
    ("F7-3",  "排單狀態色彩示意（DataGrid 各狀態列）"),
    ("F7-4",  "詳細資訊面板操作按鈕（依狀態顯示可用操作）"),
    ("F8-1",  "排單管理主畫面（統計卡片 / 日期篩選 / 排單清單 / 設備卡片牆）"),
    ("F8-2",  "接單指派對話框（設備選擇 / SOP / 數量）"),
    ("F8-3",  "設備卡片牆（各設備負載 Chip 與指派統計）"),
    ("F8-4",  "設備焦點模式（待生產 / 生產中兩欄清單）"),
    ("F9-1",  "系統設定畫面（六個頁籤概覽）"),
    ("F9-2",  "設備管理頁籤（清單 + 新增/編輯表單）"),
    ("F9-3",  "員工管理頁籤（員工清單 + 新增員工表單）"),
    ("F9-4",  "材料管理頁籤（材料清單）"),
    ("F9-5",  "錯誤清單管理頁籤（含翻譯預覽欄）"),
    ("F9-6",  "巡檢時段設定頁籤（時段清單 + 新增表單）"),
    ("F9-7",  "角色權限管理頁籤（角色清單 + 權限勾選）"),
    ("F9-8",  "系統設定頁面（業務時間 + 同步 + 補檢 + 閒置四區塊）"),
    ("F9-9",  "日誌管理區塊（設定 + 上傳 + 附件清單 + 進度條）"),
    ("F10-1",  "讀卡機設定頁面（讀卡機清單 + 新增按鈕）"),
    ("F10-2",  "新增讀卡機對話框（COM Port 選擇 + 測試按鈕）"),
    ("F10-3",  "Modbus TCP 新增設備對話框（IP / Port / Unit ID 欄位）"),
    ("F10-4",  "Modbus TCP 讀寫測試平台（位址 / 格式 / 讀寫按鈕 / 結果顯示）"),
    ("F11-1", "離線模式提示（狀態列顯示「離線」 + 橙色指示燈）"),
    ("F12-1", "登出確認畫面（倒計時 + 確認/取消按鈕）"),
    ("F12-2", "閒置倒計時提示畫面（延長登入 / 確認登出按鈕）"),
]
...
def generate_manual():
    ...
    doc.add_paragraph()
    for label, value in [
        ("文件版本", f"{VERSION_TAG}（含圖表設計 / 排單管理 / AI Agent / 出入料管理 / 授權管理 / 硬體設定 / ABB機械手 / Modbus TCP 設備支援）"),
    ...
    toc_items = [
        ...
        ("  9.5", "巡檢時段設定", ""),
        ("  9.6", "角色與權限管理", ""),
        ("  9.7", "系統參數設定（業務時間 / 排程 / 閒置登出）", ""),
        ("  9.8", "硬體設定（讀卡機 / ABB 機械手 RAPID 位址）", ""),
        ("  9.9", "日誌管理與上傳", ""),
        ("10.",     "硬體設定（讀卡機 / Modbus TCP 設備）", ""),
        ("  10.1",  "新增讀卡機", ""),
        ("  10.2",  "移除讀卡機", ""),
        ("  10.3",  "Modbus TCP 設備", ""),
        ("  10.3.1","新增 Modbus TCP 連線", ""),
        ("  10.3.2","讀寫測試平台", ""),
        ("  10.3.3","移除 Modbus TCP 設備", ""),
        ("11.", "離線模式說明", ""),
        ("12.", "登出與閒置保護", ""),
        ("13.", "授權管理", ""),
        ("  13.1", "授權狀態查看", ""),
    ...
    simple_table(
        ...
        [
            ...
            ["📊 圖表",      "戰情室看板，可自訂統計/篩選/卡片與表格圖表",        "View 權限"],
    ...
    simple_table(
        ...
        [
            ...
            ["中間 — 進度條",  "資料載入或上傳時顯示進度"],
            ["右側 — 連線狀態","🟢 已連線 / 🔴 離線（滑鼠停留顯示主資料庫連線狀態與起始時間）；讀卡機連線圖示"],
    ...
    step(3, "從「執行人員」下拉選單確認或切換實際執行調試的人員（預設為目前登入者，可依姓名或工號搜尋切換）")
    step(4, "點擊選擇後系統開始計時，卡片進入調試模式並顯示計時器")
    step(5, "調試完成後，點擊卡片上的「結束調試」按鈕")
    step(6, "系統出現刷卡或確認提示，確認後記錄上傳")
    img_placeholder("F5-7", "調試進行中的設備卡片（計時器 + 結束調試按鈕）")
    img_placeholder("F5-8", "結束調試確認畫面（刷卡或點擊確認）")
    body("對話框資訊列會顯示該機台、目前執行人員與上次帶點人員（查無紀錄時顯示「—」），方便確認交接狀況。")
    note("調試過程中可切換至其他設備進行操作，計時器持續在背景運行", kind="tip")
    note("結束調試時，執行人員、安排調試的人員或具備「編輯」權限者皆可代為刷卡結束", kind="info")

    heading3("強制安排（編輯權限）")
    body("具備「編輯」權限的使用者可開啟對話框中的「強制安排」開關，跳過一般調試流程限制，"
         "直接指定任一品項（含關鍵字搜尋與狀態篩選）與目標調試狀態，適用於現場臨時調整或修正誤操作的情境。")
    img_placeholder("F5-9", "強制安排開關與品項篩選（狀態下拉 + 關鍵字搜尋）")
    note("強制安排會直接變更品項的生產狀態，系統記錄為「已強制安排調試」，請確認品項無誤後再送出", kind="warn")
    doc.add_page_break()
    ...
    # ══════════════════════════════════════
    heading1("6. 圖表設計")
    body("圖表設計（戰情室看板）提供可自訂的生產數據視覺化頁面，支援統計列、篩選列與卡片牆／表格容器，"
         "並可透過圖表設計器自行新增、複製、編輯或刪除圖表。")
    img_placeholder("F6-1", "圖表設計頁面總覽（頁籤 / 統計列 / 篩選列 / 卡片牆或表格容器）")

    heading2("6.1 圖表頁面總覽")
    simple_table(
        ["區塊", "說明"],
        [
            ["頁籤列", "切換不同圖表；預設圖表鎖定不可刪除，確保現場永遠有畫面可看"],
            ["統計列", "顯示依設定聚合方式計算的統計數字（不受篩選列影響，反映全資料集）"],
            ["篩選列", "篩選欄位、快捷篩選按鈕、排序切換與縮放控制"],
            ["容器區", "卡片牆或表格呈現實際資料列，樣式依圖表設計決定"],
        ],
        [3.5, 12.5]
    )
    note("畫面右上角「− 100% ＋」可切換 75% / 100% / 125% / 150% 縮放檔位，設定值與帳號綁定，各圖表頁共用", kind="tip")

    heading2("6.2 篩選與排序")
    simple_table(
        ["功能", "說明"],
        [
            ["篩選欄位",     "依圖表設計指定的欄位進行條件篩選（上限 3 個）"],
            ["快捷篩選按鈕", "點擊即套用預設條件，同欄位多顆互斥、不同欄位可疊加，再次點擊取消"],
            ["排序切換",     "下拉選單選擇排序欄位，搭配升冪／降冪切換鈕"],
            ["恢復預設",     "篩選列最右側圖示，一鍵回復篩選、按鈕與排序至圖表原始設定"],
        ],
        [3.5, 12.5]
    )

    heading2("6.3 新增與編輯圖表")
    step(1, "點擊頁籤列右側「＋ 新增」按鈕（或既有圖表旁「編輯」／「複製」按鈕）進入圖表設計器")
    step(2, "設定基本資訊：圖表名稱、資料來源（設備視角／排程視角）、排序序號")
    step(3, "設定統計列：新增統計項，指定聚合方式、來源欄位與顯示標籤")
    step(4, "設定篩選列：新增篩選欄位與快捷篩選按鈕，設定排序選項與預設排序")
    step(5, "設定容器：選擇卡片或表格呈現方式，並設定對應欄位（卡片 Chip／次要資訊；表格欄位順序）")
    step(6, "左側預覽區即時反映變更，確認無誤後點擊「儲存」")
    img_placeholder("F6-2", "圖表設計器（左側預覽 / 右側基本、統計列、篩選列、容器設定）")
    note("預設圖表鎖定無法直接編輯或刪除，需先「複製」為新圖表後才能修改", kind="info")
    note("圖表名稱為必填且不可與既有圖表重複，儲存前系統會即時驗證並顯示錯誤提示", kind="warn")

    heading2("6.4 圖表管理")
    simple_table(
        ["操作", "說明"],
        [
            ["複製", "以現有圖表為基礎建立新圖表，可自由修改內容"],
            ["刪除", "移除自訂圖表（預設圖表無法刪除），需確認後執行"],
            ["匯出", "將圖表設定匯出為 JSON 檔案，便於備份或分享"],
            ["匯入", "選取 JSON 檔案匯入圖表，名稱衝突時自動加註序號"],
        ],
        [3.5, 12.5]
    )
    note("圖表數量上限為 10 個，達上限後「新增／複製／匯入」按鈕反灰無法使用", kind="warn")
    img_placeholder("F6-3", "圖表操作列（複製 / 刪除 / 匯出 / 匯入按鈕群）")

    heading2("6.5 定期刷新設定")
    body("圖表頁面預設每 120 秒自動重新查詢資料並更新統計列與容器內容，過程不影響目前的篩選與排序狀態。")
    step(1, "開啟「工具 → 系統設定 → 系統參數設定」頁籤")
    step(2, "於圖表刷新區塊啟用／停用自動刷新，並設定刷新間隔秒數")
    step(3, "點擊「套用」儲存設定")
    note("僅停留在圖表頁面時才會刷新；切換至其他頁面或圖表設計器開啟中不會觸發", kind="info")
    doc.add_page_break()
    ...
    img_placeholder("F7-1", "出入料管理主畫面（統計列 / 篩選列 / 排單清單 / 右側詳細資訊面板）")
    ...
    img_placeholder("F7-2", "建立排單對話框（件號 / 型號 / 工序 / 數量 / 備註欄位）")
    ...
    img_placeholder("F7-3", "排單狀態色彩示意（DataGrid 各狀態列）")
    ...
    img_placeholder("F8-1", "排單管理主畫面（統計卡片 / 日期篩選 / 排單清單 / 設備卡片牆）")
    ...
    img_placeholder("F8-2", "接單指派對話框（設備選擇 / SOP / 數量）")
    ...
    img_placeholder("F8-3", "設備卡片牆（各設備負載 Chip 與指派統計）")
    ...
    img_placeholder("F8-4", "設備焦點模式（待生產 / 生產中兩欄清單）")
    ...
    img_placeholder("F9-1", "系統設定畫面（六個頁籤概覽）")
    ...
    img_placeholder("F9-2", "設備管理頁籤（清單 + 新增/編輯表單）")
    ...
    img_placeholder("F9-3", "員工管理頁籤（員工清單 + 新增員工表單）")
    note("新增員工後，需指定角色才能賦予相應操作權限", kind="warn")

    heading2("9.3 材料管理")
    body("管理物料清單，包含材料名稱、廠牌、庫存資訊。")
    img_placeholder("F9-4", "材料管理頁籤（材料清單）")
    ...
    img_placeholder("F9-5", "錯誤清單管理頁籤（含翻譯預覽欄）")
    ...
    img_placeholder("F9-6", "巡檢時段設定頁籤（時段清單 + 新增表單）")

    heading2("9.6 角色與權限管理")
    ...
    img_placeholder("F9-7", "角色權限管理頁籤（角色清單 + 權限勾選）")
    ...
    img_placeholder("F9-8", "系統設定頁面（業務時間 + 同步 + 補檢 + 閒置四區塊）")

    heading2("9.8 硬體設定（系統設定中的進階參數）")
    ...
    img_placeholder("F9-9", "日誌管理區塊（設定 + 上傳 + 附件清單 + 進度條）")
    doc.add_page_break()

    # ══════════════════════════════════════
    # 10. 硬體設定（讀卡機 / Modbus TCP 設備）
    # ══════════════════════════════════════
    heading1("10. 硬體設定（讀卡機 / Modbus TCP 設備）")
    body("點擊左側導覽「設備管理」進入硬體設定，可新增 / 移除讀卡機與 Modbus TCP 設備。")
    img_placeholder("F10-1", "讀卡機設定頁面（讀卡機清單 + 新增按鈕）")
    ...
    img_placeholder("F10-2", "新增讀卡機對話框（COM Port 選擇 + 測試按鈕）")
    ...
    img_placeholder("F10-3", "Modbus TCP 新增設備對話框（IP / Port / Unit ID 欄位）")
    note("Unit ID 對應 Modbus 從站地址（Slave Address），需與 PLC 或設備的設定一致", kind="info")

    heading3("10.3.2 讀寫測試平台")
    ...
    img_placeholder("F10-4", "Modbus TCP 讀寫測試平台（位址 / 格式 / 讀寫按鈕 / 結果顯示）")

    heading3("10.3.3 移除 Modbus TCP 設備")
    ...
    step(3, "確認後系統回到登入畫面")
    img_placeholder("F12-1", "登出確認畫面（倒計時 + 確認/取消按鈕）")

    heading2("12.2 閒置自動登出")
    ...
    img_placeholder("F12-2", "閒置倒計時提示畫面（延長登入 / 確認登出按鈕）")
    ...
    heading2("13.3 Trial 模式說明")
    body("首次執行且尚未匯入授權檔時，系統自動進入 Trial 模式，提供 14 天完整功能試用。")
    simple_table(
        ["狀態", "說明"],
        [
            ["Trial（試用中）", "14 天內所有功能均可使用，導覽按鈕顯示剩餘天數提示"],
            ["Trial Expired（試用到期）", "試用期滿後，需授權功能（圖表 / 排單 / 程式庫 / 出入料 / AI 助理）進入鎖定狀態"],
        ],
        [4.5, 9.5]
    )
    doc.add_paragraph()

    heading2("13.4 授權到期處理")
    ...
    heading2("14.4 AI 回覆顯示與用量")
    bullet("AI 回覆以 Markdown 格式呈現：支援標題、清單、表格與程式碼區塊，便於閱讀結構化的查詢結果")
    bullet("回覆中的超連結僅顯示文字、不可點擊（安全性設計）")
    bullet("輸入框下方顯示本次對話累計的 Tokens 用量（輸入 / 輸出），切換對話時各自獨立累計")
    doc.add_page_break()

    # ══════════════════════════════════════
    # 15. 常見問題 FAQ
    # ══════════════════════════════════════
    heading1("15. 常見問題（FAQ）")
