...
namespace FProductionDashBoard.Models.Extra
{
    public static class MesEquipmentMapper
    {
        ...
        public static Equipment ToEquipment(MesDevice mes) => new()
        {
            ...
            TypeId   = _groupToTypeId.TryGetValue(mes.Group ?? string.Empty, out var tid) ? tid : null,
        ...
        public static bool IsSynced(Equipment eq, MesDevice mes)
        {
            ...
            return eq.Name == mes.Name
                && eq.Ip   == mes.Ip
                && expectedSerialNum == (mes.SerialNum ?? string.Empty)
                && expectedGroup     == (mes.Group ?? string.Empty)
                && (eq.Factory  ?? string.Empty) == (mes.Factory ?? string.Empty)
                && (eq.Building ?? string.Empty) == (mes.Building ?? string.Empty)
                && (eq.Floor    ?? string.Empty) == (mes.Floor ?? string.Empty);
        }
    }
}
